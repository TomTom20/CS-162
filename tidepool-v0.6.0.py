...
class moneyBot:
    ...
    def loadConfig(self):
        ...
        self.reserve = float(cfg.config["cashToReserve"])
        print("Reserved Cash: $" + str(self.reserve))
        self.buyLimitPrice = float(cfg.config["buyLimit"])
        print("Buy Limit: " + str(self.buyLimitPrice * 100) + "%")
        self.sellLimitPrice = float(cfg.config["sellLimit"])
        print("Sell Limit: " + str(self.sellLimitPrice * 100) + "%")
        self.exponentialMA = cfg.config["exponentialMovingAverage"]
        #self.movingAverageShort = cfg.config["movingAverageShort"]
        #self.movingAverageLong = cfg.config["movingAverageLong"]
        ...
        print("EMA Window: " + str(self.exponentialMA))
        #print("Short MA Window: " + str(self.movingAverageShort))
        #print("Long MA Window: " + str(self.movingAverageLong))

        #if self.rsiWindow > self.movingAverageLong:
        #    self.minConsecutiveSamples = self.rsiWindow
        #else:
        self.minConsecutiveSamples = self.exponentialMA + 1
    ...
    def buy(self, c, price):
        ...
        if (availableCash > 1.0):
            ...
            sellAt = price + (price * self.sellLimitPrice)
    ...
    def checkBuyCondition(self, c):
        ...
        oldEMA = self.data.iloc[0][str(self.coinList[c]) + "-EMA"]
        newEMA = self.data.iloc[-1][str(self.coinList[c]) + "-EMA"]
        print("Old EMA: " + str(oldEMA) + " New EMA: " + str(newEMA))
        #movingAverageS = self.data.iloc[-1][str(self.coinList[c]) + "-ShortMA"]
        #movingAverageL = self.data.iloc[-1][str(self.coinList[c]) + "-LongMA"]
        if math.isnan(oldEMA) == False and math.isnan(newEMA) == False:
            if oldEMA < newEMA and self.coinState[c].numHeld == 0.0:
    ...
    # look at values in last row only
        ...
        #RSI = self.data.iloc[-1][str(self.coinList[c]) + "-RSI"]
            ...
            #if price > self.coinState[c].purchasedPrice + (self.coinState[c].purchasedPrice * self.sellLimitPrice) and self.coinState[c].numHeld > 0.0:
                #return True
    ...
    def updateDataframe(self, now):
        ...
        for c in self.coinList:
            colName = c + "-EMA"
            self.data[colName] = self.data[c].shift(1).values[::-1].ewm(span=self.exponentialMA, adjust=False).mean()
            #colName = c + "-ShortMA"
            #self.data[colName] = self.data[c].shift(1).rolling(window=self.movingAverageShort).mean()
            #colName = c + "-LongMA"
            #self.data[colName] = self.data[c].shift(1).rolling(window=self.movingAverageLong).mean()
            #colName = c + "-RSI"
            ...
            if (now.minute in self.runMinute):
                ...
                if self.boughtIn == True:
                    for c in self.coinState:
                        if (c.timeBought != ''):
                            ...
                            if (timeDiffBuyOrder.total_seconds() > (60 * 10) and coinHeld == 0.0):
                ...
                for c in range(0, len(self.coinList)):
                    ...
                    if self.checkBuyCondition(c) and self.pricesGood == True:
                        price = self.data.iloc[-1][self.coinList[c]]
                        buyPrice = price * self.buyLimitPrice
                        self.buy(c, buyPrice)

                    if self.checkSellCondition(c) and self.pricesGood == True:
                        price = self.data.iloc[-1][self.coinList[c]]
                        sellPrice = price * self.sellLimitPrice
                        self.sell(c, sellPrice)
